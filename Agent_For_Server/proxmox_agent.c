...
void insert_cpu_usage_via_api() // Modified signature
{
    ...
    int len = snprintf(json_payload, sizeof(json_payload),
                       ...
                       loadavg,
                       timestamp);
    printf("Debug for fast api %s",fastapi_base_url);
    if (len < 0 || len >= sizeof(json_payload)) {
...
void insert_Common_info_via_api() {
    ...
    if (len < 0 || len >= sizeof(json_payload)) {
        ...
        if (asprintf(&url_buffer, "%s/physical-server/general-info/", fastapi_base_url) == -1) {
            fprintf(stderr, "[ERROR] Failed to allocate URL string\n");
        }
